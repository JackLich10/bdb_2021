...
## Sourced from GitHub repo: https://github.com/mlfurman3/gg_field
...
gg_field <- function(yardmin = 0, yardmax = 120, buffer_x = 5, buffer_y = 5, direction = "horiz",
  ...
  ## upright dimensions
  upright_length = 18.5/3
  upright_yardline = 120
  upright_accross_field_location = 160/6
  upright_color = "#E8DE35"
  upright_line_width = 2
  upright_shape = 21
  upright_size = 4
  upright_outline_color = 'black'
  ...
  p <- list(
    ...
    annotate("rect", xmin=xmin-buffer_x, xmax=xmax+buffer_x, ymin=ymin-buffer_y, ymax=ymax+buffer_y, 
             fill=field_color),
    ...
    annotate("segment", x = upright_yardline, xend = upright_yardline,
             y = upright_accross_field_location + upright_length/2, yend = upright_accross_field_location - upright_length/2,
             color = upright_color, lwd = upright_line_width),
    annotate("point",
             x = upright_yardline,
             y = upright_accross_field_location + upright_length/2,
             size = upright_size, shape = upright_shape,
             fill = upright_color, color = upright_outline_color),
    annotate("point",
             x = upright_yardline,
             y = upright_accross_field_location - upright_length/2,
             size = upright_size, shape = upright_shape,
    ...
    annotate("rect",xmin=0, xmax=xmax, ymin=ymax, ymax=ymax+buffer_y, fill=sideline_color),
    annotate("rect",xmin=0, xmax=xmax, ymin=ymin-buffer_y, ymax=ymin, fill=sideline_color),
    ...
    if (direction == "horiz") {
      coord_cartesian(xlim=c(yardmin, yardmax), ylim = c(ymin-buffer_y,ymax+buffer_y), 
                      expand = FALSE)
      
    } else if (direction == "vert") {
      ## flip entire plot to vertical orientation
      coord_flip(xlim=c(yardmin, yardmax), ylim = c(ymin-buffer_y,ymax+buffer_y), expand = FALSE)
